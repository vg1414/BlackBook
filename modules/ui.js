...
export function showScreen(screenId) {
  ...
  document.getElementById('bottom-nav').classList.toggle('hidden', screenId === 'lobby');
}
...
export function renderBalances(balances, players, currentPlayerId) {
  ...
  const items = Object.entries(players)
    .map(([id, player]) => ({ id, player, net: balances[id]?.net || 0 }))
  ...
  container.innerHTML = items.map(({ id, player, net }) => {
    ...
    return `<div class="balance-item ${cls}">
      <div class="player-avatar" style="background:${player.color}20;color:${player.color}">${player.name.charAt(0).toUpperCase()}</div>
      <div class="balance-info"><span class="balance-name">${escHtml(player.name)}${isYou ? '<span class="balance-you">(du)</span>' : ''}</span></div>
...
export function renderSettlements(balances, players) {
  ...
  if (!players || Object.keys(players).length === 0) { section.style.display = 'none'; return; }
  const netMap = {};
  Object.entries(players).forEach(([id]) => { netMap[id] = balances[id]?.net || 0; });
  const transactions = minimizePayments(netMap);
  if (transactions.length === 0) { section.style.display = 'none'; return; }
  section.style.display = 'block';
  container.innerHTML = transactions.map(t => `
    <div class="settlement-item">
      <span class="settlement-from">${escHtml(players[t.from]?.name || t.from)}</span>
      <span class="settlement-arrow">→</span>
      <span class="settlement-to">${escHtml(players[t.to]?.name || t.to)}</span>
...
export function renderActiveSessionPreview(sessions, players) {
  ...
  const active = sessions ? Object.entries(sessions).find(([, s]) => s.status === 'active') : null;
  if (!active) { container.innerHTML = '<p class="muted">Ingen aktiv session</p>'; return; }
  ...
  container.innerHTML = `<div style="display:flex;justify-content:space-between;align-items:center">
    <div><div style="font-weight:600;color:var(--gold)">${escHtml(label)}</div>
    <div style="font-size:13px;color:var(--text-muted)">${playerCount} spelare · ${typeLabel(session.type)}</div></div>
...
export function renderQuickMode(players, sessionPlayerIds) {
  ...
  const ids = sessionPlayerIds ? Object.keys(sessionPlayerIds).filter(id => players[id]) : [];
  container.innerHTML = ids.map(id => {
    const p = players[id];
    return `<div class="quick-player-row" data-player-id="${id}">
...
export function renderBuyinMode(players, sessionPlayerIds, entries) {
  ...
  container.innerHTML = ids.map(id => {
    const p = players[id];
    const pe = Object.values(entries || {}).filter(e => e.playerId === id && !e.deleted && ['buyin','rebuy','cashout'].includes(e.type));
    const totalIn = pe.filter(e => e.type === 'buyin' || e.type === 'rebuy').reduce((s, e) => s + Math.abs(e.amount), 0);
    const cashout = pe.find(e => e.type === 'cashout');
    const hasCashout = !!cashout;
    const net = hasCashout ? cashout.amount + pe.filter(e => e.type === 'buyin' || e.type === 'rebuy').reduce((s, e) => s + e.amount, 0) : null;
    const netCls = net === null ? '' : net > 0 ? 'positive' : net < 0 ? 'negative' : 'zero';
    const hasBuyin = pe.some(e => e.type === 'buyin');
    return `<div class="buyin-player-card ${hasBuyin ? 'has-buyin' : ''}" data-player-id="${id}">
      ...
      <div class="buyin-stat">Inne: ${oreToSek(totalIn)}${hasCashout ? ' · Cashout: ' + oreToSek(cashout.amount) : ''}</div>
...
export function renderHistory(sessions, players) {
  ...
  if (!sessions) { container.innerHTML = '<div class="empty-state"><div class="empty-icon">📋</div><p>Ingen historik ännu</p></div>'; return; }
  const closed = Object.entries(sessions).filter(([, s]) => s.status === 'closed').sort((a, b) => (b[1].closedAt || 0) - (a[1].closedAt || 0));
  if (closed.length === 0) { container.innerHTML = '<div class="empty-state"><div class="empty-icon">📋</div><p>Ingen historik ännu</p></div>'; return; }
  container.innerHTML = closed.map(([id, s]) => {
    const date = s.closedAt ? new Date(s.closedAt).toLocaleDateString('sv-SE') : '–';
    return `<div class="history-item" data-session-id="${id}">
      <div class="history-item-header"><span class="history-item-name">${escHtml(s.name || typeLabel(s.type))}</span><span class="history-item-date">${date}</span></div>
...
export function renderSessionDetail(session, entries, players) {
  document.getElementById('detail-session-name').textContent = session.name || typeLabel(session.type);
  const listEl = document.getElementById('detail-entries-list');
  const sessionEntries = Object.entries(entries || {}).filter(([, e]) => e.sessionId === session.id && !e.deleted).sort((a, b) => a[1].timestamp - b[1].timestamp);
  if (sessionEntries.length === 0) { listEl.innerHTML = '<p class="muted">Inga poster</p>'; return; }
  listEl.innerHTML = sessionEntries.map(([, e]) => {
    const amtCls = e.amount > 0 ? 'positive' : e.amount < 0 ? 'negative' : '';
    return `<div class="detail-entry-item">
      <div class="detail-entry-left">
        <span class="detail-entry-player">${escHtml(players[e.playerId]?.name || e.playerId)}</span>
...
export function renderGroupPlayers(players, currentPlayerId) {
  ...
  if (!players || Object.keys(players).length === 0) { container.innerHTML = '<p class="muted">Inga spelare</p>'; return; }
...
export function renderSessionPlayerSelect(players, selectedIds) {
  document.getElementById('session-player-select').innerHTML = Object.entries(players).map(([id, p]) =>
    `<button class="player-checkbox-btn ${selectedIds.includes(id) ? 'selected' : ''}" data-player-id="${id}">${escHtml(p.name)}</button>`).join('');
}

export function typeLabel(type) {
  return { poker: 'Poker', quick: 'Snabb', sport: 'Sport', other: 'Övrigt' }[type] || type || 'Session';
}
